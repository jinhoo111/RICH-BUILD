...
>
> ※ 설계 결정 메모 (2026-06-16 확정):
>    계좌 레벨 통화 설정은 PHASE 3-2에서 제거했음.
>    (계좌 하나에 국내·해외 종목을 함께 보유 가능 → 통화는 종목 레벨에서 결정)
>    현재 요약 카드는 ₩ 합계 / $ 합계 분리 표시 (임시).
>    이 Phase에서 환율 API 연동 후 → 요약 카드 [₩ KRW] / [$ USD] 단일 토글로 통합.
>    토글 시 전체 금액을 선택 통화 기준으로 즉시 환산 재계산.
